...
export default function App() {
  ...
  return (
    <div className="app-shell" style={{ '--venue-primary': activeVenue.profile.brandPrimary, '--venue-accent': activeVenue.profile.brandAccent, '--venue-surface': activeVenue.profile.brandSurface ?? '#f5f5f5', '--venue-text': activeVenue.profile.brandText ?? activeVenue.profile.brandPrimary } as CSSProperties}>
      ...
      <footer className="site-footer saas-footer"><div className="shell">{ownerAuthenticated || coupleAuthenticatedWeddingId === activeWedding?.id || page === 'venue' ? <><span>{activeVenue.profile.shortName}</span><span>Powered by Venue Visions</span></> : platformAuthenticated ? <><span>Venue Visions Admin</span><span>Internal proof of concept · {venueConfigs.length} venue profiles</span></> : <><span>Venue Visions</span><span>Event venue management & planning · venue-first private client workspaces</span></>}</div></footer>
    </div>
  )
}
