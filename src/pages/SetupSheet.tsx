...
export default function SetupSheet({ venueId, wedding }: SetupSheetProps) {
  ...
  return (
    <main className="page-main shell setup-sheet-page">
      <section className="setup-sheet-toolbar no-print"><div><p className="eyebrow">{venue.shortName.toUpperCase()} · SETUP SHEET</p><h1>Final handoff.</h1><p>Print or save this planning summary after the couple's selections and layouts are finalized.</p></div><button className="button button--primary" onClick={() => window.print()}>Print / Save PDF</button></section>
      <section className="setup-sheet paper-sheet">
        <header className="paper-sheet__header"><div className="venue-brand-mark venue-brand-mark--paper" style={{ background: venue.brandPrimary, color: '#fff' }}>{venue.logoText}</div><div><span>{venue.shortName.toUpperCase()} · POWERED BY VENUE VISIONS</span><h2>{wedding.profile.couple}</h2><p>{formatDate(wedding.profile.date)} · {pkg.name} · {wedding.profile.guests} guests</p></div></header>
        <div className="paper-summary-grid"><article><span>Ceremony</span><strong>{config.areas.find((a) => a.id === wedding.profile.ceremonyArea)?.name || 'Not selected'}</strong></article><article><span>Reception</span><strong>{config.areas.find((a) => a.id === wedding.profile.receptionArea)?.name || 'Not selected'}</strong></article><article><span>Décor pieces</span><strong>{wedding.selections.reduce((sum, item) => sum + item.quantity, 0)}</strong></article><article><span>Planning status</span><strong>{wedding.status}</strong></article></div>
        <section className="paper-block"><h3>{venue.inventoryLabel || 'Décor inventory'} pull list</h3>{selections.length ? <table><thead><tr><th>Item</th><th>Qty</th><th>Storage</th></tr></thead><tbody>{selections.map((entry) => <tr key={entry.itemId}><td>{entry.item!.name}</td><td>{entry.quantity}</td><td>{entry.item!.storage}</td></tr>)}</tbody></table> : <p>No décor selected.</p>}</section>
        ...
        <footer className="paper-sheet__footer"><span>Generated with Venue Visions.</span><span>{venue.isSample ? 'Sample venue and planning data.' : 'Configured preview; operational details should be confirmed by the venue.'}</span></footer>
      </section>
    </main>
  )
}
