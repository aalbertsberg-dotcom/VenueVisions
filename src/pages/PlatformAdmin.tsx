...
export default function PlatformAdmin({ authenticated, onAuthenticate, onLogout, onNavigate, leads, weddings, venues, onOpenVenue }: PlatformAdminProps) {
  ...
  if (!authenticated) return (
    <main className="owner-access-page shell"><section className="panel owner-access-card platform-access-card">
      <div className="owner-access-lock">VV</div><p className="eyebrow">VENUE VISIONS ADMIN · PROOF OF CONCEPT</p><h1>Shape the company side of Venue Visions.</h1>
      <p className="owner-access-lead">This internal proof of concept is for reviewing what Venue Visions should need as a company: venue accounts, preview requests, onboarding, support, billing and platform settings. Venue customers would never see this area.</p>
  ...
  return (
    <main className="page-main shell platform-page">
      <section className="page-intro page-intro--split admin-intro"><div><p className="eyebrow">VENUE VISIONS ADMIN · PROOF OF CONCEPT</p><h1>What should Venue Visions need to run the company?</h1><p>This is an internal concept for the founders to review. It now shows two separate venue profiles so the tenant model is easier to evaluate.</p></div><div className="owner-session-actions"><span className="prototype-badge prototype-badge--large">Admin POC</span><button className="text-link" onClick={onLogout}>Exit POC</button></div></section>
      ...
      <section className="metric-grid platform-metrics">
        ...
        <article><span>Wedding workspaces</span><strong>{weddings.length}</strong><small>kept separate by venue</small></article>
      ...
      <section className="panel platform-tenant-panel"><div className="panel__heading"><div><p className="eyebrow">VENUE ACCOUNTS · PROOF OF CONCEPT</p><h2>Two venues, two completely different customer experiences.</h2><p>The platform owns the common workflow while branding, spaces, packages, inventory and couples stay scoped to each venue.</p></div><button className="button button--primary button--small" onClick={() => onNavigate('for-venues')}>Open venue request form</button></div>
        <div className="platform-venue-grid">{venues.map((config) => { const venueWeddings = weddings.filter((wedding) => wedding.venueId === config.profile.id); return (
          <article className="platform-venue-card" key={config.profile.id} style={{ borderTopColor: config.profile.brandAccent }}>
            ...
            <div className="platform-venue-card__stats"><span><strong>{venueWeddings.length}</strong>weddings</span><span><strong>{config.inventory.length}</strong>inventory styles</span><span><strong>{config.packages.length}</strong>packages</span><span><strong>{config.areas.length}</strong>spaces</span></div>
      ...
      <section className="panel platform-architecture"><div><p className="eyebrow">COMPANY MODEL · PROOF OF CONCEPT</p><h2>Venue Visions remains the product.</h2><p>Each venue receives a configured account and branded portal; its couples receive private wedding workspaces inside that venue.</p></div><div className="architecture-tree"><strong>Venue Visions</strong><span>Company-owned platform</span><i>↓</i><strong>Venue accounts</strong><span>{venues.map((v) => v.profile.shortName).join(' · ')}</span><i>↓</i><div><b>Venue A couples</b><b>Venue B couples</b></div></div></section>
    </main>
  )
}
