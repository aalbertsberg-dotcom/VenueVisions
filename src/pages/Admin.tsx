...
export default function Admin({ venueId, weddings, activeWeddingId, onSelectWedding, onOpenWedding, onAddWedding, authenticated, onAuthenticate, onExitPreview, onLogout, onNavigate }: AdminProps) {
  ...
  const copyInvite = (wedding: WeddingWorkspace) => copyText(`${venue.shortName} ${eventLabel} portal: ${coupleLink(wedding)}  Preview access code: ${wedding.accessCode}`, 'Preview invite')
  ...
  return (
    <main className="page-main shell admin-page venue-admin" style={{ '--venue-primary': venue.brandPrimary, '--venue-accent': venue.brandAccent, '--venue-surface': venue.brandSurface ?? '#f4f4f4' } as CSSProperties}>
      ...
      <section className="panel admin-weddings admin-weddings--cards">
        <div className="panel__heading admin-weddings__heading"><div><p className="eyebrow">UPCOMING</p><h2>{clientPlural[0].toUpperCase() + clientPlural.slice(1)} &amp; {eventLabel} workspaces</h2><p className="booking-rule">{config.ownerDashboardNote} Venue Visions prevents duplicate dates inside this venue.</p></div><button className="button button--small button--primary" onClick={() => { setShowAdd((current) => !current); setFormError('') }}>{showAdd ? 'Cancel' : `+ Add ${eventLabel}`}</button></div>
