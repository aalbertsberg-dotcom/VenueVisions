...
export default function Admin({ venueId, weddings, activeWeddingId, onSelectWedding, onOpenWedding, onAddWedding, authenticated, onAuthenticate, onExitPreview, onLogout, onNavigate }: AdminProps) {
    ...
    return (
      <main className="owner-access-page shell">
        <section className="panel owner-access-card venue-owner-access" style={{ '--venue-primary': venue.brandPrimary, '--venue-accent': venue.brandAccent } as CSSProperties}>
          ...
          <p className="eyebrow">{venue.shortName.toUpperCase()} · OWNER PREVIEW ACCESS</p>
          <h1>Venue owner dashboard.</h1>
          <p className="owner-access-lead">This gate represents the private {venue.shortName} admin area. Production would use secure authentication and venue-specific permissions.</p>
          <form className="owner-access-form" onSubmit={(event) => { event.preventDefault(); if (!onAuthenticate(accessCode)) setAccessError('Incorrect preview password.'); else setAccessError('') }}>
          ...
          <div className="owner-access-note"><strong>Preview environment.</strong> No real customer, contract or payment information should be entered here.</div>
  ...
  const coupleLink = (wedding: WeddingWorkspace) => `${window.location.origin}${window.location.pathname}#/venue/${encodeURIComponent(venue.slug)}/couple/${encodeURIComponent(wedding.accessSlug)}`
  ...
  const copyInvite = (wedding: WeddingWorkspace) => copyText(`${venue.shortName} wedding portal: ${coupleLink(wedding)}  Preview access code: ${wedding.accessCode}`, 'Preview invite')
  ...
  return (
    <main className="page-main shell admin-page venue-admin" style={{ '--venue-primary': venue.brandPrimary, '--venue-accent': venue.brandAccent, '--venue-surface': venue.brandSurface ?? '#f4f4f4' } as CSSProperties}>
      <section className="page-intro page-intro--split admin-intro">
        <div><p className="eyebrow">{venue.shortName.toUpperCase()} · OWNER DASHBOARD</p><h1>Every couple. One venue command center.</h1><p>Switch weddings, watch booked dates, manage {inventoryTitle}, review package status and keep each couple's private workspace separate.</p></div>
      ...
      {activeWedding && <section className="panel owner-active-wedding owner-active-wedding--dynamic"><div className="owner-active-wedding__copy"><p className="eyebrow">ACTIVE WEDDING</p><h2>{activeWedding.profile.couple}</h2><p>{formatDate(activeWedding.profile.date)} · {packageById(activeWedding.profile.packageId, venueId).name}</p></div><div className="owner-active-wedding__controls"><label htmlFor="owner-active-wedding-select">Switch active couple</label><select id="owner-active-wedding-select" value={activeWeddingId} onChange={(event) => onSelectWedding(event.target.value)}>{sortedWeddings.map((wedding) => <option key={wedding.id} value={wedding.id}>{wedding.profile.couple} · {formatDate(wedding.profile.date)}</option>)}</select><div className="owner-active-wedding__buttons"><button className="button button--primary button--small" onClick={() => onOpenWedding(activeWedding.id, 'wedding')}>Open workspace</button><button className="button button--ghost button--small" onClick={() => onOpenWedding(activeWedding.id, 'messages')}>Messages</button></div></div></section>}

      <section className="admin-metrics metric-grid"><article><span>Upcoming weddings</span><strong>{weddings.length}</strong><small>inside {venue.shortName}</small></article><article><span>{inventoryTitle}</span><strong>{inventory.length}</strong><small>configured inventory styles</small></article><article><span>Selected pieces</span><strong>{totalSelected}</strong><small>across this venue</small></article><article><span>Unread messages</span><strong>{totalUnread}</strong><small>for venue team</small></article></section>

      <section className="owner-quick-grid">
        <button className="owner-quick-card" onClick={() => onNavigate('calendar')}><span>CALENDAR + MILESTONES</span><strong>See booked dates and timing</strong><small>{config.oneEventPerDate ? 'One wedding per calendar date' : 'Venue-defined availability rules'}</small><b>→</b></button>
        <button className="owner-quick-card" onClick={() => onNavigate('catalog')}><span>{inventoryTitle.toUpperCase()}</span><strong>Review décor and package access</strong><small>Counts, availability and storage</small><b>→</b></button>
        <button className="owner-quick-card" onClick={() => onNavigate('messages')}><span>MESSAGES</span><strong>Keep each wedding conversation together</strong><small>Follows the active couple</small><b>→</b></button>
      ...
      <section className="panel inventory-admin"><div className="panel__heading"><div><p className="eyebrow">{inventoryTitle.toUpperCase()}</p><h2>Storage &amp; availability</h2><p>{venue.isSample ? 'This inventory is entirely fictional and exists to demonstrate another venue configuration.' : 'Categories reflect public Chandelier Oaks information. Counts, dimensions and storage locations remain sample values until the venue catalogs the actual collection.'}</p></div><button className="button button--small button--primary">+ Add décor item</button></div><div className="admin-table-wrap"><table className="admin-table admin-table--inventory"><thead><tr><th>Item</th><th>Category</th><th>Tier</th><th>Available</th><th>Storage location</th></tr></thead><tbody>{inventory.map((item) => <tr key={item.id}><td><strong>{item.name}</strong></td><td>{item.category}</td><td>{item.accessTier}</td><td>{item.quantity}</td><td><code>{item.storage}</code></td></tr>)}</tbody></table></div></section>
    </main>
  )
}
