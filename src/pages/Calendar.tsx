...
export default function Calendar({ venueId, weddings, activeWeddingId, onSelectWedding }: CalendarProps) {
  ...
  return <main className="page-main shell calendar-page">
    <section className="page-intro page-intro--split"><div><p className="eyebrow">{venue.shortName.toUpperCase()} · OWNER TOOL</p><h1>Calendar &amp; planning milestones.</h1><p>See booked dates at a glance and keep venue-specific timing attached to each wedding without turning Venue Visions into accounting software.</p></div><div className="calendar-rule-card"><span>BOOKING RULE</span><strong>One wedding per calendar date</strong><small>{config.ownerDashboardNote}</small></div></section>
    <div className="calendar-layout">
      ...
      <aside className="panel milestone-panel"><div className="panel__heading"><div><p className="eyebrow">UPCOMING WEDDINGS</p><h2>Milestone tracker</h2></div></div><div className="milestone-wedding-list">{sorted.map((wedding)=>{ const pkg=packageById(wedding.profile.packageId,venueId); const milestones=paymentMilestones(wedding,chandelier); return <article key={wedding.id} className={wedding.id===activeWeddingId?'milestone-wedding milestone-wedding--active':'milestone-wedding'} onClick={()=>onSelectWedding(wedding.id)}><div className="milestone-wedding__heading"><div><strong>{wedding.profile.couple}</strong><span>{fmt(dateOnly(wedding.profile.date))}</span></div><b>${pkg.price.toLocaleString()}</b></div><span className="milestone-package">{pkg.name}</span><div className="milestone-steps">{milestones.map((m,i)=><div className={i<wedding.paymentStepsCompleted?'milestone-step milestone-step--done':'milestone-step'} key={m.label}><i>{i<wedding.paymentStepsCompleted?'✓':i+1}</i><span><strong>{m.label}</strong><small>{m.date?fmt(m.date):'At contract signing'} · {m.detail}</small></span></div>)}</div></article>})}</div><div className="calendar-demo-note"><strong>{chandelier?'Chandelier Oaks timing:':'Sample timing:'}</strong> {chandelier?'The first milestone and 270/180/60-day schedule follow the venue’s public FAQ. Later installment amounts and paid/unpaid statuses are not public, so they are not invented here.':'Juniper & Stone is fictional; its milestone schedule exists only to demonstrate that every venue can configure different timing.'}</div></aside>
    </div>
  </main>
}
