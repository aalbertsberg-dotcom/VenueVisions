    ...
    <>
      <header className={`site-header app-header app-header--${mode}`} style={{ '--venue-primary': activeVenue.brandPrimary, '--venue-accent': activeVenue.brandAccent } as CSSProperties}>
        <div className="app-header__brand">
          ...
          {mode === 'couple' && <VenueBrand venue={activeVenue} onClick={() => go('wedding')} subtitle="Wedding Portal · Powered by Venue Visions" />}
        </div>
        ...
        <div className="app-header__actions">
          {mode === 'owner' && <label className="owner-wedding-switcher" title="Switch the active wedding across all owner tools"><span>Active wedding</span><select value={activeWeddingId} onChange={(event) => onSelectWedding(event.target.value)}>{sortedWeddings.map((wedding) => <option value={wedding.id} key={wedding.id}>{wedding.profile.couple} · {wedding.profile.date}</option>)}</select></label>}

          {mode === 'couple' && <div className="profile-menu-wrap"><button className="couple-profile-button" type="button" aria-expanded={profileOpen} onClick={() => setProfileOpen((current) => !current)}><span>{activeWeddingName}</span><b aria-hidden="true">⌄</b></button>{profileOpen && <div className="couple-profile-popover"><span className="mini-label">{activeVenue.shortName.toUpperCase()}</span><strong>{activeWeddingName}</strong><button onClick={() => go('wedding')}>Wedding home</button><button onClick={() => go('media')}>Media & inspiration</button><button onClick={() => go('summary')}>Setup summary</button><button onClick={() => go('venue')}>{activeVenue.shortName} home</button><button onClick={() => { setProfileOpen(false); onCoupleLogout() }}>Sign out</button></div>}</div>}
      ...
      <aside className={menuOpen ? 'clean-drawer clean-drawer--open' : 'clean-drawer'} aria-label="Application menu">
        <div className="clean-drawer__heading"><div><span className="mini-label">{mode === 'owner' || mode === 'couple' ? activeVenue.shortName.toUpperCase() : 'VENUE VISIONS'}</span><strong>{mode === 'owner' ? 'Owner Portal' : mode === 'couple' ? activeWeddingName : mode === 'platform' ? 'Admin · Proof of Concept' : 'Venue Visions'}</strong></div><button type="button" onClick={() => setMenuOpen(false)} aria-label="Close menu">×</button></div>

        {mode === 'owner' && <div className="clean-drawer__switcher"><label htmlFor="drawer-active-wedding">Active wedding</label><select id="drawer-active-wedding" value={activeWeddingId} onChange={(event) => onSelectWedding(event.target.value)}>{sortedWeddings.map((wedding) => <option value={wedding.id} key={wedding.id}>{wedding.profile.couple} · {wedding.profile.date}</option>)}</select><small>All wedding-specific tools follow this selection inside {activeVenue.shortName}.</small></div>}
      ...
      {mode === 'couple' && <nav className="couple-bottom-nav" aria-label="Wedding navigation"><button className={page === 'wedding' ? 'active' : ''} onClick={() => go('wedding')}><span>⌂</span><small>Home</small></button><button className={page === 'catalog' ? 'active' : ''} onClick={() => go('catalog')}><span>✿</span><small>Décor</small></button><button className={page === 'planner' ? 'active' : ''} onClick={() => go('planner')}><span>▦</span><small>Design</small></button><button className={page === 'messages' ? 'active' : ''} onClick={() => go('messages')}><span>◌</span><small>Messages</small></button><button onClick={() => setProfileOpen((current) => !current)}><span>☰</span><small>More</small></button></nav>}
    </>
  )
}
